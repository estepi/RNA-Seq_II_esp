---
title: "Análisis de datos de RNA-Seq I"
...
El diseño experimental es una comparación entre **múltiples  condiciones**, con **XX réplicas** biológicas cada una.

Antes de empezar armamos un archivo *targets* o *mapping file* donde se detalla cada muestra a qué condición corresponde 

Empezaremos con una compracion entre 2 condiciones: 

```{r targets, echo=TRUE, eval=TRUE}
targets <- read.table("~/Documents/BioApps/data/HNRPC/targets.txt", header = T, row.names = 1)
```
...
counts <-
    read.table(
        "~/Documents/BioApps/data/HNRPC/genes.hits.txt",
        row.names = 1,
        header = T,
        stringsAsFactors = F
    )
